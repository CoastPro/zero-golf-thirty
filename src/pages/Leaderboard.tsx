        ...
        <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
          <button
            ...
            className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${
              selectedFlight === null ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
            All
          </button>
          {tournament.flights.map(flight => (
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${
                selectedFlight === flight ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
              }`}
            >
              Flt {flight}
            </button>
        ...
        <div className="flex justify-between items-center px-4 py-4 mb-3">
          <div className="w-1/2 flex justify-start">
            {tournament.leaderboard_logo_left && (
              <img src={tournament.leaderboard_logo_left} alt="Sponsor" className="h-20 object-contain" />
            )}
          </div>
          <div className="w-1/2 flex justify-end">
            {tournament.leaderboard_logo_right && (
              <img src={tournament.leaderboard_logo_right} alt="Sponsor" className="h-20 object-contain" />
            )}
          </div>
        </div>
      )}
            ...
            <table className="w-full text-xs">
              <thead className="bg-green-600 text-white">
                <tr>
                  ...
                  {showNet && (
                    <>
                      <th className="px-1.5 py-1.5 text-center font-semibold">Net</th>
                      <th className="px-1.5 py-1.5 text-center font-semibold">±</th>
                    </>
                  )}
                </tr>
                ...
                {rankedGrossLeaderboard.map((entry) => {
                  ...
                  return (
                    <tr key={entry.player.id} className={`hover:bg-gray-50 ${isLeader ? 'bg-yellow-50' : ''}`}>
                      ...
                      {tournament.show_handicaps && (
                        <td className="px-1.5 py-1.5 text-center">{entry.player.handicap}</td>
                      )}
                      ...
                      {showNet && (
                        <>
                          <td className="px-1.5 py-1.5 text-center font-bold">{entry.netScore !== null ? entry.netScore : '-'}</td>
                          <td className="px-1.5 py-1.5 text-center font-semibold">{entry.vsParNet !== null ? formatVsPar(entry.vsParNet) : '-'}</td>
                        </>
                      )}
                    </tr>
                  );
      ...
      {activeTab === 'stableford' && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-gray-200">
                {rankedStablefordLeaderboard.map((entry) => {
                  const isLeader = entry.rank === '1';
                  const vsQuotaDisplay = entry.isComplete
                    ? entry.vsQuota.toFixed(0)
                    : entry.vsQuota.toFixed(1);
                  return (
                    <tr key={entry.player.id} className={`hover:bg-gray-50 ${isLeader ? 'bg-yellow-50' : ''}`}>
                      <td className="px-1.5 py-1.5 font-semibold">
                        {entry.rank}
                      ...
                      <td className="px-1.5 py-1.5 font-medium truncate max-w-[100px]">{entry.player.name}</td>
                      {tournament.show_quotas && (
                        <td className="px-1.5 py-1.5 text-center">{entry.player.quota}</td>
                      )}
                      ...
                      <td className="px-1.5 py-1.5 text-center font-semibold">
                        {entry.holesPlayed > 0 ? `${entry.vsQuota > 0 ? '+' : ''}${vsQuotaDisplay}` : '-'}
                      </td>
                    </tr>
                  );
                })}
          ...
          <div className="bg-gray-50 px-2 py-1.5 text-xs text-gray-600 border-t">
            * Prorated during play, final vs full quota when complete
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-lg shadow p-3 mb-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              <div className="text-center p-2 bg-green-50 rounded">
                <div className="text-lg font-bold text-green-600">${skinsData.totalPot.toFixed(2)}</div>
                <div className="text-xs text-gray-600">Total Pot</div>
              </div>
              <div className="text-center p-2 bg-blue-50 rounded">
                <div className="text-lg font-bold text-blue-600">{skinsData.skinsWon}</div>
                <div className="text-xs text-gray-600">Skins Won</div>
              </div>
              <div className="text-center p-2 bg-yellow-50 rounded">
                <div className="text-lg font-bold text-yellow-600">${skinsData.valuePerSkin.toFixed(2)}</div>
                <div className="text-xs text-gray-600">Per Skin</div>
              </div>
              <div className="text-center p-2 bg-purple-50 rounded">
                <div className="text-lg font-bold text-purple-600">{tournament.skins_type.toUpperCase()}</div>
              ...
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-gray-200">
                  {skinsData.leaderboard.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-2 py-4 text-center text-gray-600">No skins won yet</td>
                    </tr>
                  ) : (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                {skinsData.winners.map(winner => (
                  <div key={`${winner.hole}-${winner.playerId}`} className="border rounded p-2">
                    ...
                    {winner.skinsWon > 1 && (
                      <div className="text-xs font-bold text-green-600">{winner.skinsWon} Skins!</div>
                    )}
                  </div>
                ))}
              </div>
